...
type DeployModalProps = {
  onClose: () => void;
  onDeploy: (params: { repo: string; appName: string }) => Promise<void>;
  isDeploying: boolean;
};
...
function AppRow({ app, onAction, loadingKeys, onSelect }: AppRowProps): ReactNode {
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", gap: "10px", marginTop: "24px" }}>
          ...
          <button
            onClick={() => canSubmit && onDeploy({ repo, appName })}
            disabled={!canSubmit}
            style={{
              flex: 2,
              padding: "10px",
...
export default function DeployDashboard(): ReactNode {
  ...
  const loadAppLogs = async (appName: string): Promise<void> => {
    setLoadingLog(true);
    try {
      const res = await fetch(`/api/logs?app=${appName}`, { headers: getAuthHeaders() });
      if (res.status === 401) { router.push("/login"); return; }
      if (!res.ok) { setDeployLog("Failed to load logs."); return; }
      const data = await res.json();
      setDeployLog(data.logs || "No logs found.");
    } finally {
      setLoadingLog(false);
    }
  };
  ...
  useEffect(() => {
    if (activeTab !== "logs" || !selectedApp) return;
    loadAppLogs(selectedApp);
    const interval = setInterval(() => loadAppLogs(selectedApp), 5000);
    return () => clearInterval(interval);
  ...
  const handleDeploy = async ({ repo, appName }: { repo: string; appName: string }): Promise<void> => {
    setIsDeploying(true);
    ...
    try {
      const res = await fetch("/api/action", {
        method: "POST",
        headers: getAuthHeaders(),
        body: JSON.stringify({ action: "deploy", repo, appName }),
      });
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "160px 80px 100px 1fr auto", alignItems: "center", gap: "16px", padding: "14px 20px", borderBottom: "1px solid #141414", background: "#0b0d10" }}>
                      <div
                        onClick={() => {
                          ...
                          setDeployLog("");
                          setActiveTab("logs");
                          loadAppLogs(app.name);
                        }}
                        style={{ color: "#e8e8e8", fontFamily: "monospace", fontSize: "13px", fontWeight: 600, cursor: "pointer" }}
                        title="View deploy logs"
                      >
                        {app.name}
                      </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
            ...
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
                <div style={{ color: "#444", fontSize: "11px", letterSpacing: "0.1em", textTransform: "uppercase" }}>
                  deploy log —{" "}
                  {selectedApp
                    ? <span style={{ color: "#3a6fff" }}>{selectedApp}</span>
                    : <span style={{ color: "#333" }}>click an app to view</span>
                  }
                </div>
                {loadingLog && <Spinner color="#3a6fff" size={9} />}
              </div>
              <div style={{
                background: "#0a0a0a",
                border: "1px solid #1e1e1e",
                ...
                {!selectedApp
                  ? <span style={{ color: "#333" }}>no app selected</span>
                  : deployLog || "Loading..."
                }
              </div>
            </div>
            ...
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
                <div style={{ color: "#444", fontSize: "11px", letterSpacing: "0.1em", textTransform: "uppercase" }}>session output</div>
                <button
                  onClick={() => setLogs([])}
        ...
        <div style={{ marginTop: "24px", display: "flex", gap: "10px", flexWrap: "wrap" }}>
          {[
            ...
            },
            {
              ...
              action: async () => {
                setActiveTab("logs");
                addLog([{ type: "cmd", text: "pm2 status" }]);
                try {
                  const res = await fetch("/api/status", { headers: getAuthHeaders() });
                  if (res.status === 401) { router.push("/login"); return; }
                  const data = await res.json();
                  const freshApps: App[] = data.apps || [];
                  setApps(freshApps);
                  addLog(
                    freshApps.length === 0
                      ? [{ type: "info", text: "no apps found" }]
                      : freshApps.map((a) => ({
                        type: a.status === "online" ? "success" : "error",
                        text: `${a.name.padEnd(16)} ${a.status.padEnd(10)} :${a.port}  uptime: ${a.uptime}`,
                      }))
                  );
                } catch (err: any) {
                  addLog([{ type: "error", text: err.message || "failed to fetch status" }]);
                }
              },
            },
